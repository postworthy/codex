...
#[cfg(not(debug_assertions))]
pub use update_action::get_update_action_for_cached_version;
mod update_prompt;
#[cfg(any(not(debug_assertions), test))]
mod update_versions;
mod updates;
...
mod tests {
    ...
    #[tokio::test]
    async fn embedded_logs_db_corruption_is_recovered_locally() -> color_eyre::Result<()> {
        let temp_dir = TempDir::new()?;
        let mut config = build_config(&temp_dir).await?;
        ...
        let state_db = init_state_db_for_app_server_target(&config, &AppServerTarget::Embedded)
            .await?
            .expect("embedded startup should initialize the state db");
        state_db.close().await;
        assert!(logs_db_path.exists());
        Ok(())
    }
